...
func TestGenerateOneTimeAddress(t *testing.T) {
	...
	for _, uaddr := range vailidUaddrs {
		ota, err := s.GenerateOneTimeAddress(ctx, uaddr)
		if err != nil {
			t.Errorf("uaddr:%s, err:%s", uaddr, err.Error())
		}

		if len(ota) != common.UAddressLength*2+2 {
			t.Errorf("invalid ota length! uaddr:%s, ota:%s", uaddr, ota)
		}

		if ota[0] != '0' || (ota[1] != 'x' && ota[1] != 'X') {
			t.Errorf("invalid ota! uaddr:%s, ota:%s", uaddr, ota)
		}
	}
	...
	for _, uaddr := range invalidUaddr {
		ota, err := s.GenerateOneTimeAddress(ctx, uaddr)
		if err == nil {
			t.Errorf("succeed from invalid useaddress. uaddr:%s, ota:%s", uaddr, ota)
		}
	}
}
